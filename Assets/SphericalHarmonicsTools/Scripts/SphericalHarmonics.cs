...
public class SphericalHarmonics
{
    ...
    public static bool GPU_Project_Uniform_9Coeff(RenderTexture input, Vector4[] output)
    {        
        int mip = 4;
        int width = input.width / Mathf.RoundToInt(Mathf.Pow(2, mip));

        //the starting number of groups 
        int ceiled_size = Mathf.CeilToInt(width / 8.0f);
        ...
        desc.bindMS = false;
        desc.colorFormat = input.format;// ConvertRenderFormat(input.format);
        ...
        for (int face = 0; face < 6; ++face)
            Graphics.CopyTexture(input, face, mip, converted_input, face, 0);

        //cycle 9 coefficients
        for (int c = 0; c < 9; ++c)
        {
            ceiled_size = Mathf.CeilToInt(width / 8.0f);
            ...
            reduce.SetInt("input_size", width);
