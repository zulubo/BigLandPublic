...
public static class ComputeHelper
{
	...
	public static void CreateRenderTexture3D(ref RenderTexture texture, int sizeX, int sizeY, int sizeZ, GraphicsFormat format, TextureWrapMode wrapMode = TextureWrapMode.Repeat, string name = "Untitled", bool mipmaps = false)
	{
		if (texture == null || !texture.IsCreated() || texture.width != sizeX || texture.height != sizeY || texture.volumeDepth != sizeZ || texture.graphicsFormat != format)
		{
			//Debug.Log ("Create tex: update noise: " + updateNoise);
			if (texture != null)
			{
			...
			const int numBitsInDepthBuffer = 0;
			texture = new RenderTexture(sizeX, sizeY, numBitsInDepthBuffer);
			texture.graphicsFormat = format;
			texture.volumeDepth = sizeZ;
			texture.enableRandomWrite = true;
			texture.dimension = UnityEngine.Rendering.TextureDimension.Tex3D;
			texture.useMipMap = mipmaps;
			texture.autoGenerateMips = false;
