...
uniform half overcast;

#define PI 3.14159265359

#define TRANSMITTANCE_LUT_SPACEPOS 0.8
#define TRANSMITTANCE_LUT_SPACESIZE 0.2

//blue noise
sampler2D _BlueNoise;
static const float NOISETEXSIZE = 512;
float SampleBlueNoise(uint2 uv) {
	return tex2Dlod(_BlueNoise, float4(uv.xy / NOISETEXSIZE, 0, 0)).r;
}
...
ScatteringParameters getScatteringValues(float3 rayPos) {
	...
	float3 rayleigh = lerp(rayleighCoefficients, 0.2, overcast * 0.99) * rayleighDensity;
...
float3 getSunTransmittance(float3 pos, float3 sunDir) {
	const int sunTransmittanceSteps = 10;
...
float3 getSunTransmittanceLUT(sampler2D lut, float3 pos, float3 dir) {
	...
	float uvX = dot(pos / dstFromCentre, dir) * 0.5 + 0.5;

	if (height01 < 1) 
	{
		// in atmosphere
		height01 = saturate(height01) * TRANSMITTANCE_LUT_SPACEPOS;
	}
	else 
	{
		// in space
	...
	return tex2Dlod(lut, float4(uvX, saturate(height01), 0, 0)).rgb;
}

ScatteringResult raymarch(float3 rayPos, float3 rayDir, float rayLength, int numSteps, sampler2D transmittanceLUT, uint2 uv, float earthShadowRadius) {
	float3 luminance = 0;
	float3 transmittance = 1;
	...
	rayPos += rayDir * SampleBlueNoise(uv) * stepSize;
	...
	for (int stepIndex = 0; stepIndex < numSteps; stepIndex ++) {
		...
		//if (overcast) {
		//	sunTransmittance = saturate(dot(normalize(rayPos), normalize(dirToSun)));
		//}
		//else {
			sunTransmittance = getSunTransmittanceLUT(transmittanceLUT, rayPos, dirToSun);
		//}
	...
	result.luminance = luminance * sunColor;
